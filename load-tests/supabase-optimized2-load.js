// =====================================================================
// KISSARIYA — LOAD TEST (ROUND-2 OPTIMIZED WORKLOAD — AFTER)
// =====================================================================
// Strict comparison with load-tests/supabase-optimized-load.js (round-1
// AFTER = round-2 BEFORE): same real Supabase project, same browsing order,
// same sleep() pauses, same VU stages, read-only, anon key only.
//
// ONLY the app query shape differs (round 2 changes):
//   * catalog products  : no count=exact, limit=17 (pageSize 16 + 1 probe)
//     -> hasNextPage is detected via the extra row, no COUNT(*) exact.
//   * search products   : no count=exact, limit=17 (same reason).
//   * slug->id lookups  : already resolved client-side in round-1 k6
//     (ids passed directly) — unchanged, matching the round-2 app which
//     resolves from the React Query cache.
// Everything else is byte-identical to round-1.
// =====================================================================
...
// ---- Identical select strings to round-1 (app service) ----
...
export default function (data) {
  ...
  // ---- Page: Catalog with category filter (ROUND-2: no count=exact, limit=17) ----
  ...
  // ---- Search (ROUND-2: no count=exact, limit=17) ----
...
export function handleSummary(data) {
  ...
  return {
    [`load-tests/results/optimized2-${vu}vu.json`]: JSON.stringify(data, null, 2),
  };
}
