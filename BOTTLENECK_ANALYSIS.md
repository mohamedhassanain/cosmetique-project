...
| 6 | Supabase API (edge/PostgREST gateway) | **UNKNOWN — REQUIRES LIVE DASHBOARD METRICS** | Traffic is 100% Supabase REST; Docker/Nginx excluded at same VUs. Real observability during load: bandwidth/API-latency panels not captured. Whether the collapse is the gateway vs a tenant-level limit cannot be told from k6 alone. |
| 7 | PostgreSQL | **PROVEN — NOT the bottleneck** | Real Supabase database metrics during load: **CPU ≈2%, memory ≈55%, peak connections ≈15/60, disk IO ≈0%, disk usage ≈14%**. Index coverage for every load query is PROVEN present (supabase/database.sql); catalog ~1–10 active rows. |
| 8 | Supabase platform / rate limits | **LIKELY — tenant-level request/latency limit (not DB, not PG pool)** | 0% HTTP errors (no 429/5xx observed) while latency collapses to 60s timeouts, with DB CPU ≈2% and connections 15/60 ⇒ the DB and pool are idle; the constraint sits above PostgreSQL (edge/gateway/tenant throttling). To become PROVEN: capture the Supabase dashboard API-latency/RPS panels during a 1000 VU run (SUPABASE_LOAD_MONITORING.md §2–§4). |
| 9 | Query inefficiency | **PROVEN — NOT the bottleneck** | No select=*, no count=exact, no N+1 in the public app; indexes match every query (SUPABASE_LOAD_AUDIT.md §3–§4); `site_settings` SELECT=* and home products `limit=60` exist ONLY in the k6 script, not in the public app (SUPABASE_REQUEST_MAP.md / REQUEST_AMPLIFICATION_ANALYSIS.md). |
